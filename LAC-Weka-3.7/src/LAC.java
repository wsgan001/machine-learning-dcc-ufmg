 ...
 */
public class LAC extends AbstractClassifier implements AdditionalMeasureProducer, TechnicalInformationHandler
{
	...
	public TechnicalInformation getTechnicalInformation()
	{
		...
		result.setValue(Field.
		result.setValue(Field.SCHOOL, "Federal University of Minas Gerais");
		result.setValue(Field.ADDRESS, "Belo Horizonte, BR");

		additional = result.add(Type.ARTICLE);
		additional.setValue(Field.AUTHOR, "Adriano Veloso, Wagner Meira Jr., Mohammed Zaki");
		additional.setValue(Field.YEAR, "2006");
		additional.setValue(Field.TITLE, "Lazy Associative Classification");
		additional.setValue(Field.JOURNAL, "ICDM '06 Proceedings of the Sixth International Conference on Data Mining");
		additional.setValue(Field.PAGES, "645-654");
		additional.setValue(Field.ISBN, "0-7695-2701-9");
		additional.setValue(Field.
	...
	/** pruning strategy */
	public static final Tag[] TAGS_PRUNING = { new Tag(PRUNING_UNPRUNED, "unpruned", "Un-pruned"),
			new Tag(PRUNING_POSTPRUNING, "postpruned", "Post-pruning"), new Tag(PRUNING_PREPRUNING, "prepruned", "Pre-pruning") };

	/** the pruning strategy */
	protected int m_PruningStrategy = PRUNING_POSTPRUNING;

	/** Successor nodes. */
	protected LAC[] m_Successors;
	...
	/**
	 * Fixed number of expansions (if no pruning method is used, its value is
	 * -1. Otherwise, its value is gotten from internal cross-validation).
	 */
	...
	/**
	 * If use Gini index as the splitting criterion - default (if not,
	 * information is used).
	 */
	...
	public void buildClassifier(Instances data) throws Exception
	{
		...
		if (m_PruningStrategy == PRUNING_UNPRUNED)
		{
			...
			// Compute information of the best split for this node (include
			// split attribute,
			// split value and gini gain (or information gain)). At the same
			// time, compute
			...
			FastVector nodeInfo = computeSplitInfo(this, data, sortedIndices, weights, dists, props, totalSubsetWeights, m_Heuristic,
					m_UseGini);
			...
			m_Expansion = 0;
			makeTree(BestFirstElements, data, sortedIndices, weights, dists, classProbs, totalWeight, props[attIndex], m_minNumObj,
					m_Heuristic, m_UseGini, m_FixedExpansion);
		...
		// weights,
		// class probabilities, class distributions, branch proportions and
		// total subset
		...
		LAC[] m_roots = new LAC[m_numFoldsPruning];
		...
		for (int i = 0; i < m_numFoldsPruning; i++)
		{
			...
			nodeInfo[i] = computeSplitInfo(m_roots[i], train[i], sortedIndices[i], weights[i], dists[i], props[i], totalSubsetWeights[i],
		...
		if (m_PruningStrategy == PRUNING_PREPRUNING)
		{
			...
			while (true)
			{
				...
				for (int i = 0; i < m_numFoldsPruning; i++)
				{
					...
					{
						if (m_roots[i] == null)
							continue; // if the tree cannot be expanded, go to
										// next fold
						m_roots[i].m_isLeaf = false;
						LAC nodeToSplit = (LAC) (((FastVector) (parallelBFElements[i].elementAt(0))).elementAt(0));
						if (!m_roots[i].makeTree(parallelBFElements[i], m_roots[i], train[i], nodeToSplit.m_SortedIndices,
								nodeToSplit.m_Weights, nodeToSplit.m_Dists, nodeToSplit.m_ClassProbs, nodeToSplit.m_TotalWeight,
								nodeToSplit.m_Props, m_minNumObj, m_Heuristic, m_UseGini))
						{
		...
		else
		{
			...
			for (int i = 0; i < m_numFoldsPruning; i++)
			{
				...
				m_roots[i].makeTree(parallelBFElements[i], m_roots[i], train[i], test[i], modelError[i], nodeToSplit.m_SortedIndices,
						nodeToSplit.m_Weights, nodeToSplit.m_Dists, nodeToSplit.m_ClassProbs, nodeToSplit.m_TotalWeight,
						nodeToSplit.m_Props, m_minNumObj, m_Heuristic, m_UseGini, m_UseErrorRate);
				m_roots[i] = null;
			}
			...
			for (int i = 0; i < maxExpansion; i++)
			{
				...
				error[i] = error[i] / counts[i]; // average error for each
													// expansion
		...
		FastVector prune_nodeInfo = computeSplitInfo(this, data, prune_sortedIndices, prune_weights, prune_dists, prune_props,
		...
		m_Expansion = 0;
		makeTree(BestFirstElements, data, prune_sortedIndices, prune_weights, prune_dists, prune_classProbs, prune_totalWeight,
				prune_props[attIndex], m_minNumObj, m_Heuristic, m_UseGini, expansion);
	}
	 ...
	 * method). Pre-pruning and post-pruning methods also use this method to
	 * build the final tree on all training data based on the expansion
	 * calculated from internal cross-validation.
	 * 
	 * @param BestFirstElements
	 *            list to store BFTree nodes
	 * @param data
	 *            training data
	 * @param sortedIndices
	 ...
	 * @param dists
	 *            class distributions for each attribute
	 * @param classProbs
	 *            class probabilities of this node
	 * @param totalWeight
	 *            total weight of this node (note if the node can not split,
	 *            this value is not calculated.)
	 * @param branchProps
	 *            proportions of two subbranches
	 * @param minNumObj
	 *            minimal number of instances at leaf nodes
	 * @param useHeuristic
	 *            if use heuristic search for nominal attributes in multi-class
	 *            problem
	 * @param useGini
	 *            if use Gini index as splitting criterion
	 * @param preExpansion
	 *            the number of expansions the tree to be expanded
	 * @throws Exception
	 *             if something goes wrong
	 */
	protected void makeTree(FastVector BestFirstElements, Instances data, int[][] sortedIndices, double[][] weights, double[][][] dists,
			double[] classProbs, double totalWeight, double[] branchProps, int minNumObj, boolean useHeuristic, boolean useGini,
			int preExpansion) throws Exception
	{
		...
		// /////////////////////////////////////////////////////////////////////
		// All information about the node to split (the first BestFirst object
		// in
		// BestFirstElements)
		...
		// If no enough data or this node can not be split, find next node to
		// split.
		if (totalWeight < 2 * minNumObj || branchProps[0] == 0 || branchProps[1] == 0)
		{
			...
			if (BestFirstElements.size() != 0)
			{
				...
				LAC nextSplitNode = (LAC) nextSplitElement.elementAt(0);
				nextSplitNode.makeTree(BestFirstElements, data, nextSplitNode.m_SortedIndices, nextSplitNode.m_Weights,
						nextSplitNode.m_Dists, nextSplitNode.m_ClassProbs, nextSplitNode.m_TotalWeight, nextSplitNode.m_Props, minNumObj,
		...
		// information gain.
		// (namely, gini gain or information gain of all nodes in
		// BestFirstEelements is 0).
		if (gain == 0 || preExpansion == m_Expansion)
		{
			for (int i = 0; i < BestFirstElements.size(); i++)
			{
				...
				LAC node = (LAC) element.elementAt(0);
				node.makeLeaf(data);
			}
		...
		else
		{
			...
			else
			{
				...
				if ((m_PruningStrategy == PRUNING_PREPRUNING) || (m_PruningStrategy == PRUNING_POSTPRUNING) || (preExpansion != -1))
					m_Expansion++;
	 ...
	 * node in the BestFirstElements if it is expansible, otherwise it looks for
	 * next exapansible node. If it finds a node is expansibel, expand the node,
	 * then return true. (note it just expands one node at a time).
	 * 
	 * @param BestFirstElements
	 *            list to store BFTree nodes
	 * @param root
	 *            root node of tree in each fold
	 * @param train
	 *            training data
	 * @param sortedIndices
	 *            sorted indices of the instances
	 ...
	 *             if something goes wrong
	 */
	protected boolean makeTree(FastVector BestFirstElements, LAC root, Instances train, int[][] sortedIndices, double[][] weights,
			double[][][] dists, double[] classProbs, double totalWeight, double[] branchProps, int minNumObj, boolean useHeuristic,
			boolean useGini) throws Exception
	{
		...
		// node to split
		LAC nodeToSplit = (LAC) firstElement.elementAt(0);
		...
		// If no enough data to split for this node or this node can not be
		// split find next node to split.
		if (totalWeight < 2 * minNumObj || branchProps[0] == 0 || branchProps[1] == 0)
		{
			// remove the first element
			BestFirstElements.removeElementAt(0);
			...
			LAC nextNode = (LAC) ((FastVector) BestFirstElements.elementAt(0)).elementAt(0);
			return root.makeTree(BestFirstElements, root, train, nextNode.m_SortedIndices, nextNode.m_Weights, nextNode.m_Dists,
					nextNode.m_ClassProbs, nextNode.m_TotalWeight, nextNode.m_Props, minNumObj, useHeuristic, useGini);
		}

		// If gini gain or information is 0, make all nodes in the
		// BestFirstElements leaf nodes
		...
		else
		{
			...
			splitData(subsetIndices, subsetWeights, nodeToSplit.m_Attribute, nodeToSplit.m_SplitValue, nodeToSplit.m_SplitString,
			...
			else
			{
				...
				nodeToSplit.makeSuccessors(BestFirstElements, train, subsetIndices, subsetWeights, dists, nodeToSplit.m_Attribute,
	 ...
	 * @param test
	 *            test data in each fold
	 * @param modelError
	 *            list to store error for each expansion in each fold
	 * @param sortedIndices
	 *            sorted indices of the instances
	 * @param weights
	 *            weights of the instances
	 ...
	 *             if something goes wrong
	 */
	protected void makeTree(FastVector BestFirstElements, LAC root, Instances train, Instances test, FastVector modelError,
			int[][] sortedIndices, double[][] weights, double[][][] dists, double[] classProbs, double totalWeight, double[] branchProps,
			int minNumObj, boolean useHeuristic, boolean useGini, boolean useErrorRate) throws Exception
	{
		...
		if (totalWeight < 2 * minNumObj || branchProps[0] == 0 || branchProps[1] == 0)
		{
			...
			LAC nextSplitNode = (LAC) ((FastVector) BestFirstElements.elementAt(0)).elementAt(0);
			nextSplitNode.makeTree(BestFirstElements, root, train, test, modelError, nextSplitNode.m_SortedIndices,
					nextSplitNode.m_Weights, nextSplitNode.m_Dists, nextSplitNode.m_ClassProbs, nextSplitNode.m_TotalWeight,
					nextSplitNode.m_Props, minNumObj, useHeuristic, useGini, useErrorRate);
			return;
	 ...
	 * 
	 * @param BestFirstElements
	 *            list to store BestFirst nodes
	 * @param data
	 *            training instance
	 * @param subsetSortedIndices
	 *            sorted indices of instances of successor nodes
	 * @param subsetWeights
	 *            weights of instances of successor nodes
	 * @param dists
	 *            class distributions of successor nodes
	 * @param att
	 *            attribute used to split the node
	 * @param useHeuristic
	 *            if use heuristic search for nominal attributes in multi-class
	 *            problem
	 * @param useGini
	 ...
	 *             if something goes wrong
	 */
	protected void makeSuccessors(FastVector BestFirstElements, Instances data, int[][][] subsetSortedIndices, double[][][] subsetWeights,
			double[][][] dists, Attribute att, boolean useHeuristic, boolean useGini) throws Exception
	{

		m_Successors = new LAC[2];

		for (int i = 0; i < 2; i++)
		{
			m_Successors[i] = new LAC();
			...
			double[][] totalSubsetWeights = new double[data.numAttributes()][2];
			FastVector splitInfo = m_Successors[i].computeSplitInfo(m_Successors[i], data, subsetSortedIndices[i], subsetWeights[i],
			...
			// sorted indices and weights of each attribute for this successor
			// node
			...
			// insert this successor node into BestFirstElements according to
	...
	protected double computeSortedInfo(Instances data, int[][] sortedIndices, double[][] weights, double[] classProbs) throws Exception
	{
		...
		for (int j = 0; j < data.numAttributes(); j++)
		{
			...
			else
			{
				...
				// missing values instances are put to end (through Utils.sort()
				// method)
				for (int i = 0; i < data.numInstances(); i++)
				{
					Instance inst = data.instance(i);
	 ...
	 *            class distributions for each attribute
	 * @param props
	 *            proportions of two branches
	 * @param totalSubsetWeights
	 *            total weight of two subsets
	 * @param useHeuristic
	 *            if use heuristic search for nominal attributes in multi-class
	 *            problem
	 * @param useGini
	 ...
	 */
	protected FastVector computeSplitInfo(LAC node, Instances data, int[][] sortedIndices, double[][] weights, double[][][] dists,
			double[][] props, double[][] totalSubsetWeights, boolean useHeuristic, boolean useGini) throws Exception
	{
		...
		for (int i = 0; i < data.numAttributes(); i++)
		{
			...
			{
				// nominal attribute
				splitString[i] = nominalDistribution(props, dists, att, sortedIndices[i], weights[i], totalSubsetWeights, gains, data,
						useHeuristic, useGini);
			}
		}
	 ...
	 * 
	 * @param props
	 *            proportions of each two branches for each attribute
	 * @param dists
	 *            class distributions of two branches for each attribute
	 * @param att
	 *            numeric att split on
	 * @param sortedIndices
	 *            sorted indices of instances for the attirubte
	 * @param weights
	 *            weights of instances for the attirbute
	 * @param subsetWeights
	 *            total weight of two branches split based on the attribute
	 * @param gains
	 *            Gini gains or information gains for each attribute
	 * @param data
	 *            training instances
	 * @param useGini
	 *            if use Gini index as splitting criterion
	 ...
	 *             if something goes wrong
	 */
	protected double numericDistribution(double[][] props, double[][][] dists, Attribute att, int[] sortedIndices, double[] weights,
			double[][] subsetWeights, double[] gains, Instances data, boolean useGini) throws Exception
	{
	 ...
	 *             if something goes wrong
	 */
	protected String nominalDistribution(double[][] props, double[][][] dists, Attribute att, int[] sortedIndices, double[] weights,
			double[][] subsetWeights, double[] gains, Instances data, boolean useHeuristic, boolean useGini) throws Exception
	{
		...
		if (data.numClasses() == 2)
		{
			...
			// // Firstly, for attribute values which class frequency is not
			// zero
		...
		else
		{
			...
			int[] numInstancesValue = new int[n]; // number of instances for an
													// attribute value
			double[] meanClass = new double[k]; // vector of mean class
												// probability
		...
		// Then, for the attribute values that class frequency is 0, split it
		// into the
	 ...
	 * 
	 * @param subsetIndices
	 *            sorted indecis of instances for each attribute for two
	 *            successor node
	 * @param subsetWeights
	 *            weights of instances for each attribute for two successor node
	 * @param att
	 *            attribute the split based on
	 * @param splitPoint
	 *            split point the split based on if att is numeric
	 * @param splitStr
	 *            split subset the split based on if att is nominal
	 * @param sortedIndices
	 *            sorted indices of the instances to be split
	 * @param weights
	 *            weights of the instances to bes split
	 * @param data
	 *            training data
	 * @throws Exception
	 *             if something goes wrong
	 */
	protected void splitData(int[][][] subsetIndices, double[][][] subsetWeights, Attribute att, double splitPoint, String splitStr,
			int[][] sortedIndices, double[][] weights, Instances data) throws Exception
	{
	 ...
	 * 
	 * @param parentDist
	 *            class distributions of parent node
	 * @param childDist
	 *            class distributions of successor nodes
	...
	protected String toString(int level)
	{
		...
		if (m_Attribute == null)
		{
			...
			{
				double correctNum = Math.rint(m_Distribution[Utils.maxIndex(m_Distribution)] * 100) / 100.0;
				double wrongNum = Math.rint((Utils.sum(m_Distribution) - m_Distribution[Utils.maxIndex(m_Distribution)]) * 100) / 100.0;
